...
| `er reconcile` | none | `ER_CONFIG`, lake env | `0`; `1` non-convergence or CONTRADICTION-1; `3` mixed `model_version` above `review_low`; `10` empty affected set or an unchanged reconciliation plan (including cuts and review escalations) | `affected_entities, entities_created, entities_merged, entities_split, entities_retired, edges_cut` |
...
- Splink receives that same connection via `DuckDBAPI(connection=<conn>, output_schema='splink_scratch')`, so no `__splink__` relation ever lands in DuckLake. Input frames are read fully qualified (`lake.main.int_std_records`) or materialized into local temp tables first; **only final scored pairs are inserted into `lake.main.match_scores`, through one MERGE**. Full matching also retires obsolete active scores in the same transaction (S4.3.4); no intermediate prediction batches are committed.
...
**`content_hash` (normative).** `content_hash` is the SHA-256, hex-encoded lowercase, of the UTF-8 concatenation of the source columns named in `sources.<name>.columns`, **in the declared order**, each value NFC-normalized and joined by the `0x1f` unit separator, with NULL encoded as the empty string. It excludes `ingested_at`, `ingest_batch_id` and `std_version`. Two implementations that disagree on this definition make T-IDEM-1a and T-IDEM-1 non-reproducible; it is therefore pinned here. `er.ingest.landing.content_hash(row, columns)` remains the reference implementation; native ingestion uses the parity-tested DuckDB expression generated by `er.ingest.hashing.content_hash_sql`. Both encode the same rendered source values.

**Metadata-aware delivery versions.** Ingestion additionally supplies the sorted names of
unmapped client fields to both hash implementations. With no metadata the digest above is
unchanged. Otherwise, let `H` be that mapped-field digest and `S` the unit separator.
For each metadata field, in sorted name order, concatenate `sha256(name)` and either
`N` for a null value or `V + sha256(value)` for a present value; metadata names and values
are hashed as verbatim UTF-8, without normalization. The delivered `content_hash` is
`sha256(H + S + "metadata-v1" + S + concatenated_fields)`. Field presence, null, empty
strings and field renames are distinct. This makes metadata-only corrections new versions
without adding metadata to matching features. The existing historical-version anti-join
and replay semantics still apply. The source ID, update timestamp and fixture `persona_id`
are not metadata. Values retain the adapters' existing text/null representation.
...
**Client metadata.** `source_metadata(payload, spec)` collects fields not consumed by
any of the nine canonical mappings, the source record ID or the source update timestamp.
The reserved fixture field `persona_id` is excluded. Source field names and JSON values
are preserved; keys are sorted, and an empty result is `{}`. This `metadata JSON` column
passes through all three staging models into `int_std_records`. Extra mappings with
unsupported canonical names do not consume a field. Metadata is excluded from the
`MATCHING_RECORD_COLUMNS` projection used by training, scoring and term frequencies;
blocking and comparison configuration may not reference it.
...
**T-BLK-1** is the parity check: on `base_10`, the DISTINCT canonicalised pair set derived from `int_blocking_keys` equals Splink's blocked pair set exactly. Full matching and both incremental passes override artifact prediction rules with the current config generator; learned parameters remain frozen. Tests also exercise stale artifact rules through the production scoring paths.
...
Full matching replaces the complete active score set: in the same transaction as its MERGE, it retires every previously active row absent from the new retained predictions, including rows from older model/TF generations. This applies to pairs removed by blocking changes or falling below `review_low`, even when endpoint content is unchanged. Historical rows remain queryable. Incremental matching continues to merge only its batch results.

`er correct` executes full match with refreshed TF, full reconcile, and full assemble under the tenant lock. It never ingests, standardizes, or trains; changed standardization/blocking keys must be rebuilt first. The target TF snapshot is frozen and recorded on the run atomically before scoring, reused on retries, and activated atomically with full score replacement. A pending correction blocks other pipeline mutations until `er correct --resume RUN_ID` finishes. Assertion edits and review resolution remain available under the same tenant lock until reconciliation succeeds, so an operator can repair a contradiction before resuming. After reconciliation succeeds, these edits wait for the correction to finish, preventing an assembly-only resume from missing a new assertion. Lake maintenance remains available. Resume preserves the run's config, model, and TF identity, restarts the first unfinished or missing stage, and refuses a completed run. Corrections always reconcile the entire corpus, including old-old score changes with no ingest seeds.
...
**Edge adjustment (pre-clustering).** For each `active` assertion: `always` inserts an edge with `match_probability = 1.0` and `evidence = {"source":"assertion","assertion_id":…}`; `never` removes the edge between the endpoints regardless of score. **Assertion edges are never persisted to `match_scores`** — they are materialized into the in-memory clustering edge set only, by the assertion-adjustment step S4.5.1 names in the trailing annotation under its query (`-- then assertion-adjusted per S4.4`). The adjustment is performed on local DuckDB relations: minus active `never` pairs, plus active `always` pairs at `p = 1.0`. No assertion edge is written to the lake score table. The pure collection implementation remains a parity oracle. `assertions` is their durable record. Consequently every `match_scores` row is scored: `model_version` and `tf_snapshot_id` are `NOT NULL` there (S5), and no consumer needs a NULL-`model_version` branch.
...
**Cuts are a deterministic derivation of the current assertion-adjusted graph, not permanent constraints on subsequent cut searches.** Reconcile expands affected nodes to a fixpoint through threshold edges before cuts and previous entity memberships. It runs the cut algorithm afresh on the reached components; full runs and corrections include every current record and prior membership. Only the newly derived cuts are excluded from clustering. Persisted cuts remain an audit trail: unchanged pair/assertion/probability/scoring-generation cuts retain their IDs, obsolete cuts are released, and newly required cuts and events are inserted in the same transaction as membership. Replaying an unchanged graph writes no duplicate cut event. Retraction, content changes, or new paths can change the derived cut set even when the active assertion set is unchanged.
...
closure(S) = S
           ∪ threshold neighbours of S in the assertion-adjusted graph BEFORE cuts
           ∪ all prior entity members of records in S
affected_nodes = least fixpoint of closure starting at seed
affected_entities = prior entities intersecting affected_nodes
```

With no active cuts or never assertions, one threshold-partner join followed by whole-prior-entity expansion computes this closure: each previous entity is an entire threshold component and each changed edge touches a seed. With cuts, expansion continues to a fixpoint because a new path may change an old cut across prior entity boundaries. Full matching forces the next reconciliation to include every current record and prior membership, even without ingest seeds.
...
-- derive cuts afresh on this graph, then exclude ONLY those derived cuts
```
...
*Proof sketch.* Before assertion cuts, every changed scored edge touches a batch record under the stated preconditions. Affected-node discovery takes the transitive closure of these seeds through the current threshold graph before cuts and prior entity memberships. Consequently every component whose deterministic cut outcome can change is included in full. Both modes derive cuts from the same current graph and ordered assertions, without applying historical cuts first. The remaining components are unchanged. Thus the resulting partitions agree, including when a new path changes which old-old edge is cut.
...
**`golden_records.metadata`** aggregates metadata from every current member, independent
of attribute winners, as `{source_system: {source_record_id: metadata_object}}`. Records
without extra fields are omitted; entities without metadata receive `{}`. Source and
record keys are sorted. The record-ID layer preserves conflicting values from multiple
records in the same source. Metadata is not a survivable attribute and has no
`golden_lineage` winner. A rebuild recomputes it from current membership, so removed
members and deleted records contribute nothing.
...
Assembly retries with the same `run_id` retain its saved `er_touched_entities` entries and union them with newly detected changes, recomputing dispositions from current entity status. The replacement set commits atomically before dbt starts. A metadata match in `golden_records` does not prove that downstream marts finished: re-executing the run repeats its saved work even when that mart already committed. A separate run does not inherit completed runs' touched sets; resume a failed assembly with its original `run_id`.

**Touched-only assembly.** `assemble.py` includes entities whose current member metadata differs from the stored golden JSON (including missing golden rows), together with the set computed over this run's `entity_events` — `{entity_id : an event of type created, member_added, member_removed, merged, split, retired or edge_cut exists with this run_id}` — and writes it to `er_touched_entities(run_id, entity_id, disposition)` with `disposition ∈ {rebuild, retire}`. `retire` covers merge losers, emptied split fragments and retired entities; everything else is `rebuild`. dbt is invoked with **only** `--vars '{run_id: <ulid>}'`; the marts join `er_touched_entities` filtered on that `run_id`. Passing the ULID list itself as a var is forbidden: at the 1m scale tens of thousands of 26-char ULIDs exceed Linux's 128 KB per-argv-element limit and the invocation hard-fails with `E2BIG`.
...
**`--resume <run_id>`.** `er run-all --resume <run_id>` reads `run_stages` for that run, identifies the first stage whose `status` is not `succeeded`, and restarts the chain from there with the original `run_id`, `config_hash` and `model_version`. It refuses (exit `2`) if the config on disk hashes differently from the `runs.config_hash` of the run being resumed, or (exit `3`) if the run is already `succeeded`. Resume re-executes the failed stage in full; it never resumes mid-stage. `er correct --resume RUN_ID` uses the correction chain and also detects a missing stage after a crash between stages. Its target TF snapshot is journaled atomically with the freeze; closing a failed run cannot overwrite that journal with stale in-memory state.
...
CREATE TABLE IF NOT EXISTS lake.main.cut_edges (
  ...
  released_run_id    VARCHAR,              -- set when this cut is no longer derived
...
stg_<source>(
  ...
  birth_date DATE, metadata JSON, updated_at_source TIMESTAMP,
...
The **eleven** columns from `given_name` through `birth_date` are the **survivable column set** of `golden_records`; `survivorship_version` and `assembled_at` are provenance stamps, not survivable attributes, and `entity_id` is the key. `metadata` is aggregated from all current members and is not survivable. Normative: that survivable column set is exactly the key set of `survivorship:` in S6, with `address` expanding to the six `addr_*` columns (S6.1 V2 enforces the equality; S4.6 owns the rule that those six come from one winning record). `golden_display` is presentation casing only and MUST never be read by the matching layer.
...
**GOLDEN_SURVIVABLE_COLUMNS.** Also defined once in `src/er/lake/columns.py`, as the ordered tuple of `golden_records` columns produced by survivorship — that is, every column of `golden_records` except `entity_id`, `metadata`, `survivorship_version` and `assembled_at`:
...
- **dbt-owned relations** evolve through their enforced contract. A contract violation fails `dbt build`, which fails the stage with exit code `1`. Every incremental dbt model sets `on_schema_change='append_new_columns'`, so an additive column introduced by a `std_version` bump propagates instead of being silently dropped.
...
| V6 | Every column referenced by `blocking[].expr` and every key of `comparisons` exists in the `int_std_records` column list of S5 and is not `metadata`; `variant_match` additionally requires `name_variants` | `columns.unknown` |
...
<a id="s10-6"></a>
### 10.6 Performance experiments and promotion

Measure initial loads **including training**, incremental deliveries with a reused model, and full corrections separately. Compare uncapped EM with deterministic 1M, 5M and 10M pair caps before attributing initial-load cost to prediction blocking. Training's exact surname/postcode rule is independent of the configured prediction `name_postal` rule. Record per-stage wall/CPU time, memory and spill, candidate counts, block-size distributions, and all S8.5 quality families.

`benchmarks/performance_campaign.py` alternates arm order on a pinned image, holds corpus seed/profile and resource limits constant, and evaluates each held-out dataset independently. Promotion requires at least three 10M repetitions. Cluster precision must not decrease; cluster pairwise recall may fall by at most **0.01 percentage points** (fraction 0.0001). Spending any recall requires at least **20% end-to-end time reduction** for the target workload. Repeatable slowdowns in another workload keep the arm experimental. This label-quality budget never relaxes INV-EQ, INV-SCORE, D1 or D2.

The original generator remains the `baseline` profile. The versioned `hard-v1` profile adds missing names/addresses, unseen nicknames, given-name and surname-prefix typos, swapped names, movers with changed contacts, and shared household contacts. Its rates are recorded in `profile.json`. Use separate tuning and held-out seeds; neither synthetic profile establishes production accuracy. Exact contact agreement is a weak positive slice, not ground truth: reused/shared contacts and temporal changes require negative examples and steward validation.

Benchmark-only blocking arms compare current rules, removal/narrowing of `name_postal`, trigram MinHash, and optional CPU ONNX projection bands. Equality keys must be functions of the individual record and frozen recipe; narrower similarity acceptance uses **more rows/bits per band**, while more bands increase collision probability and candidates. Adding keys by union cannot reduce candidates; candidate reduction must come from a replacement/narrowing arm. Approximate HNSW retrieval is a diagnostic comparator, never a recall oracle or production candidate path. Small exact similarity searches or labeled pairs are the reference.

Dense inference lives only in an optional benchmark image. `benchmarks/vector-requirements.lock` pins and hashes its complete additional environment; `benchmarks/vector_runtime.py` pins the BGE model revision and artifact checksums, bakes at build time and verifies offline at runtime. The production dependency set and fourteen configuration blocks remain unchanged pending evidence. No corpus-wide row processing is added to the CLI. Production vector activation, frozen stoplist refresh, cache persistence and coherence require a later spec amendment and their parity/equivalence tests; benchmark keys must not silently activate them.
...
- Phase 2 is conditional on the S10.6 measurements. A real scorer uses the shared, content-addressed vector store and core DuckDB array functions, with offline CPU ONNX inference if dense vectors justify their cost. DuckDB VSS is not required. Adding a vector store requires its own schema/configuration contract before implementation.
- Coherence only creates review evidence. Evaluate outlier detection precision/recall and review yield on labeled bad clusters. Cluster precision can improve only after explicit steward corrections; reporting findings alone cannot be credited as an automatic precision improvement.
...
| **M6 — Phase 2** | embedding coherence implementation replacing `NoopScorer` | none | measured detection of false merges and useful review yield on held-out bad clusters; report cluster precision before/after explicit steward corrections separately; coherence findings land as `review_queue` rows with `subject_type='entity'` |
...
| D5 | `never_match` semantics | Partition-level rather than edge-level: the cut algorithm and history-independent rederivation over affected pre-cut components are normative in S4.4.2 | **LOCKED** |
