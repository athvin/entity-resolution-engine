...
**Topology: shared Postgres cluster with one catalog schema per tenant (`ER_LAKE_METADATA_SCHEMA=t_{org}`), shared S3 bucket with a per-tenant prefix (`ER_LAKE_DATA_PATH=s3://er-lake/t_{org}/`).** The engine's own definition of a tenant is exactly this (metadata schema, data path) pair — the SaaS uses the engine's isolation unit rather than inventing one. Dedicated bucket + STS-scoped credentials become an enterprise-tier option. Per-tenant Postgres instances don't pay for themselves at $2.5k–10k/yr.
...
- **Lifecycle:** provision (org row → create schema → `provision` job runs `er init` → seed config template → issue keys) → active → suspended (schedules off, writes rejected, reads allowed) → purge (optional export, drop schema, delete prefix, delete secrets; 30-day grace).
