...
## Splink 5 upgrade and next experiments

The engine pins `splink[pyarrow]==5.0.0.dev5`. Input registration, full and
incremental prediction, clustering and stored evidence now use that version's
interfaces. Frozen term-frequency lookups stay in local DuckDB tables instead of
being collected into Python dictionaries. Saved evidence retains its existing
Bayes-factor fields by converting Splink 5's log2 comparison weights in SQL.
Blocking rules, thresholds and training configuration are unchanged.

Existing models remain usable: a fixed-model comparison against Splink 4.0.16
produced the same 12 pairs and comparison evidence, with a maximum probability
difference of `1.11e-16`. Five integration checks using the old fixture model
passed, including full/incremental score parity and review evidence. The new
fixture model was trained on the pinned 10k corpus, passed seven quality/review
checks, and regenerated byte for byte in a separate training run. Its frozen TF
CSV is unchanged. Retraining changes fitted parameters because Splink 5 changes
u-sampling; this is distinct from scoring an existing frozen model.

[Robin Linacre's Splink 5 notes](https://gist.github.com/RobinL/c6d56a27d8f83c40b6b09643c0fa5d14)
describe adaptive u-sampling, leaner blocking SQL and deferred TF joins that the
upgrade can use without new configuration. The pinned release's source also
contains opt-in EM sampling, chunked prediction and DuckDB Parquet materialisation.
Their benefit to this pipeline remains an experiment, not an established speedup.

The priorities below follow from our measured ten-million-record workload:

| Priority | Experiment | Acceptance evidence |
|---|---|---|
| 1 | Expose a per-rule EM `max_pairs` setting through the validated, hashed training configuration. Start by comparing 1m and 5m caps on the surname/postcode rule, which generated 491.6m pairs and dominated training. The cap is an approximate sample target, not a hard memory bound. | Repeat training with fixed inputs; compare warnings, rare comparison levels, edge/cluster accuracy, gray-band decisions, training time and spill. Sampling changes the model, so speed alone cannot justify a default. |
| 2 | Compare ordinary full prediction with 2×2 and 4×4 chunk grids, using the same saved model and frozen TF snapshot. | Equal canonical pairs, scores within numerical tolerance, evidence, reviews and final memberships; measure peak memory, spill and runtime. Chunking adds repeated work and may be slower on small inputs. |
| 3 | Test Parquet intermediates with a private scratch directory for each run, especially alongside chunking. | Verify cleanup on success/failure and lake isolation; measure disk use and I/O as well as memory. DuckDB execution still needs memory. |
| 4 | Profile fuzzy comparisons before changing their SQL. | Demonstrate repeated function evaluation is material, then preserve null handling, level boundaries and score symmetry in any rewrite. |

Keep exact candidate counting for parity and quality validation; sampled blocking
analysis would be suitable for planning estimates only. Keep the configured
review threshold: raising it discards review candidates and does not remove the
underlying comparison work. Registered blocked-pair reuse and distributed prediction
are lower priorities while the measured bottleneck is training on one machine.
Our existing SQL profiler already captures Splink queries through the connection,
so a second profiler is useful only if it supplies missing diagnostic evidence.

These options are not enabled by the compatibility migration. Test at 100k and 1m
before another 10m run, recording input hashes, model/TF versions, resources,
accuracy and stage timings. The 10m accuracy results below remain an independent
quality issue; completion alone is not an acceptance criterion for a sampled model.
