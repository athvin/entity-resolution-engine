...
{%- macro survivorship_decision(attribute, chain, relation='member_rows') -%}
  ...
  {%- set projections = [] -%}
  {%- set ordering = [] -%}
  {%- set rival = ['record_key := record_key'] -%}
  {%- set comparisons = [] -%}
  {%- for rule in rules -%}
    ...
    {%- for term in terms -%}
      ...
      {%- set _ = rival.append(alias ~ ' := ' ~ alias) -%}
...
_ranked AS (
    SELECT
        *,
        lead(struct_pack({{ rival | join(', ') }})) OVER _order AS _r
    FROM _keyed
    WINDOW _order AS (PARTITION BY entity_id ORDER BY {{ ordering | join(', ') }})
    QUALIFY row_number() OVER _order = 1
)
SELECT
    _w.entity_id,
    _w.value,
