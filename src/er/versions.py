...
_ROWS: Final[tuple[Pin, ...]] = (
    ...
    Pin("pandas", "3.0.5", "pandas", asserted_by_doctor=False),
...
#: The deliberate Splink 5 migration and the contracts that guard it (S13).
SPLINK_MIGRATION_NOTE: Final[MigrationNote] = MigrationNote(
    ...
    blast_radius=("src/er/matching/", "src/er/entities/cluster.py", "tests/helpers/"),
    acceptance_tests=("T-INC-3", "T-BLK-1", "T-MATCH-SYM"),
    rationale=(
        "Splink 5 requires db_api.register inputs and replaces incremental inference "
        "with predict_between + predict_within. Frozen TF and prediction frames are "
        "registered explicitly; score_pair replaces compare_two_records in symmetry "
        "checks. Candidate, score and clustering parity remain acceptance gates. "
        "The exact development release is pinned while compatibility is evaluated."
    ),
)
